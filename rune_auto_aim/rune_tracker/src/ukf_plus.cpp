...
UKF_PLUS::UKF_PLUS(bool verbose, bool use_laser, bool use_radar, double std_a, double std_yawdd) {
 ...
 */
void UKF_PLUS::Init(bool verbose, bool use_laser, bool use_radar, double std_a, double std_yawdd) {
    n_x_ = 5;    // we use 5 state variables: px, py, v_abs, yaw, yawrate
    n_aug_ = 7;  // augmented state vector additionally includes std_a and std_yawdd

    // 激光 (except during init)
    use_laser_ = use_laser;

    // 雷达 (except during init)
  ...
  Complete the initialization. See UKF_PLUS.h for other member properties.
...
void UKF_PLUS::Clear() {
    UKF_PLUS::Init(verbose_, use_laser_, use_radar_, std_a_, std_yawdd_);
}
 ...
 */
void UKF_PLUS::ProcessMeasurement(MeasurementPackage meas_package) {
    /**
  TODO:
  ...
  */
    ...
    dt = (meas_package.timestamp_ - time_us_);  // time in seconds
    time_us_ = meas_package.timestamp_;
    if (verbose_) {
        cout << "now.timestamp " << meas_package.timestamp_ << "  time_us_ " << time_us_ << endl;
    ...
    if (verbose_) {
        ...
        cout << "x_ = " << endl
             << x_.format(CleanFmt) << endl;
        cout << "P_ = " << endl
             << P_.format(CleanFmt) << endl
             << endl;
    }
}
 ...
 */
void UKF_PLUS::Prediction(double delta_t) {
    /**
  TODO:
  ...
  */
    ...
    for (int i = 0; i < 2 * n_aug_ + 1; i++) {  // iterate over sigma points
        ...
        // angle normalization
        x_diff(3) = fmod(x_diff(3), 2.0 * M_PI);  // 映射至0-2pi
        P_ += weights_(i) * x_diff * x_diff.transpose();
    }

    if (verbose_) {
        cout << "x_aug: " << endl
             << x_aug.format(CleanFmt) << endl;
        cout << "Xsig_pred_: " << endl
             << Xsig_pred_.format(CleanFmt) << endl;
        cout << "x_: " << endl
             << x_.format(CleanFmt) << endl;
        cout << "P_: " << endl
             << P_.format(CleanFmt) << endl;
 ...
 */
void UKF_PLUS::UpdateLidar(MeasurementPackage meas_package) {
    /**
  TODO:
 ...
 * @param {MeasurementPackage} meas_package
 */
void UKF_PLUS::UpdateLidarUnscented(MeasurementPackage meas_package) {
    /**
  TODO:
 ...
 * @param {MeasurementPackage} meas_package
 */
void UKF_PLUS::UpdateRadar(MeasurementPackage meas_package) {
    /**
  TODO:
...
void UKF_PLUS::write_vec(const vector<double>& vec) {
